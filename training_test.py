...
class RandomizedFetchWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        u     = self.unwrapped         # type: MujocoFetchPickAndPlaceEnv
        model = u.model
        data  = u.data
        utils = u._utils
        ...
        print("distance", np.linalg.norm(block_pos - goal_pos))
        print("distance threshold", u.distance_threshold)
...
# 4) Pass the wrapped env to SAC
model = SAC("MultiInputPolicy", env, verbose=1)
model.learn(total_timesteps=200_000)

# 5) Later, evaluation automatically uses the same fix
obs, info = env.reset()
for _ in range(200):
